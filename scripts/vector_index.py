...
async def index_documents_with_vectors(
    json_file: str, collection: str = "vectors", commit: bool = True
):
    """
    ...
    for i, doc in enumerate(documents):
        ...
        if "date_indexed" in doc_copy and isinstance(doc_copy["date_indexed"], str):
            ...
            doc_copy["date_indexed"] = time.strftime(
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                print(
                    f"Successfully indexed {len(docs_with_vectors)} documents with vectors"
                )
                return True
            else:
                print(
                    f"Error indexing documents: {response.status_code} - {response.text}"
                )
                return False
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Index documents with vector embeddings"
    )
    parser.add_argument("json_file", help="Path to the JSON file containing documents")
    parser.add_argument(
        "--collection", "-c", default="vectors", help="Solr collection name"
    )
    parser.add_argument(
        "--no-commit",
        dest="commit",
        action="store_false",
        help="Don't commit after indexing",
    )
    ...
    result = await index_documents_with_vectors(
        args.json_file, args.collection, args.commit
    )
