...
async def get_collection_status(collection: str) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Error getting collection status: {response.status_code} - {response.text}"
            )
            return {}
...
async def get_document_count(collection: str) -> int:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Error getting document count: {response.status_code} - {response.text}"
            )
            return 0


async def get_document_sample(
    collection: str, num_docs: int = 3
) -> list[dict[str, Any]]:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Error getting document sample: {response.status_code} - {response.text}"
            )
            return []


async def test_text_search(
    collection: str, field: str, search_term: str
) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Error testing text search: {response.status_code} - {response.text}"
            )
            return {}
...
async def diagnose_collection(collection: str, search_term: str = "bitcoin") -> None:
    ...
    if doc_count == 0:
        print(
            "No documents found in the collection. Please index some documents first."
        )
        return
    ...
    if schema:
        ...
        for name, field in fields.items():
            ...
            if field_type and ("text" in field_type.lower() or field_type == "string"):
                ...
                print(
                    f"  - {name} (type: {field_type}, indexed: {indexed}, stored: {stored})"
                )
            ...
            if "analysis" in analysis:
                for key, stages in analysis.get("analysis", {}).items():
                    ...
                    for stage in stages:
                        if "text" in stage:
                            print(
                                f"    - {stage.get('name', 'unknown')}: {stage.get('text', [])}"
                            )
...
async def main() -> None:
    ...
    parser.add_argument(
        "--term", "-t", default="bitcoin", help="Search term to test with"
    )
