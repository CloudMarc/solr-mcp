...
async def search_by_text(query: str, collection: str | None = None, rows: int = 5):
    """
    ...
    try:
        ...
        result = await solr_client.search(query=query, collection=collection, rows=rows)
...
async def search_by_vector(query: str, collection: str | None = None, k: int = 5):
    """
    ...
    try:
        ...
        result = await solr_client.vector_search(
            vector=embedding, collection=collection, k=k
        )
    ...
    try:
        ...
        if "docs" in data and isinstance(data["docs"], list):
            ...
            for i, doc in enumerate(docs, 1):
                ...
                if "tags" in doc:
                    tags = (
                        doc["tags"] if isinstance(doc["tags"], list) else [doc["tags"]]
                    )
...
async def main():
    ...
    parser.add_argument(
        ...
        "-v",
        action="store_true",
        help="Use vector search instead of text search",
    )
    parser.add_argument(
        "--collection", "-c", default="documents", help="Collection name"
    )
    parser.add_argument(
