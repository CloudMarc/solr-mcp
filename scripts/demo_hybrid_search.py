...
async def hybrid_search(
    ...
    try:
        ...
        args = {"query": query, "blend_factor": blend_factor, "rows": rows}

        if collection:
            args["collection"] = collection

        print(f"Hybrid searching for: '{query}' with blend_factor: {blend_factor}")
        print("(0.0 = keyword only, 1.0 = vector only)\n")

        result = await c.request({"name": "solr_hybrid_search", "arguments": args})
...
async def keyword_search(
    query: str, collection: str | None = None, rows: int = 5
) -> None:
    """
    ...
    try:
        ...
        result = await c.request({"name": "solr_search", "arguments": args})
...
async def vector_search(
    ...
    try:
        ...
        result = await c.request({"name": "solr_vector_search", "arguments": args})
...
async def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Demo for hybrid search with MCP server"
    )
    ...
    parser.add_argument(
        "--mode",
        "-m",
        choices=["keyword", "vector", "hybrid", "compare"],
        default="hybrid",
        help="Search mode",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--rows", "-r", type=int, default=5, help="Number of results to return"
    )
