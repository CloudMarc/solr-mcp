...
async def direct_solr_test():
    ...
    client = SolrClient(config)  # Pass the config to SolrClient

    # Test standard search with different query formats
    print("\n=== Testing direct Solr client search with different query formats ===")
    # TODO: replace client.search() with something that ACTUALLY EXISTS!
    ...
    async with httpx.AsyncClient() as http_client:
        response = await http_client.get(
            ...
            params={"q": 'content:"double spend"~5', "wt": "json"},
        )
