    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                print(
                    f"Error getting collections: {response.status_code} - {response.text}"
                )
                return
            ...
            if "collections" in collections_data:
                ...
                for collection in collections:
                    ...
                    if schema_response.status_code != 200:
                        print(
                            f"Error getting schema for {collection}: {schema_response.status_code}"
                        )
                        continue
                    ...
                    if vector_type:
                        ...
                        print(
                            f"  Vector Dimension: {vector_type.get('vectorDimension')}"
                        )
                        print(
                            f"  Similarity Function: {vector_type.get('similarityFunction')}"
                        )
                    else:
                        print(
                    ...
                    if vector_fields:
                        ...
                        for field in vector_fields:
                            print(
                                f"    - {field.get('name')} (indexed: {field.get('indexed')}, stored: {field.get('stored')})"
                            )
                    else:
