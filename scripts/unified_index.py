...
async def index_documents(
    json_file: str, collection: str = "unified", commit: bool = True
):
    """
        ...
        for i, doc in enumerate(solr_docs):
            ...
            try:
                response = await client.post(
                    solr_url, json=doc, params=params, timeout=30.0
                )

                if response.status_code != 200:
                    print(
                        f"Error indexing document {doc['id']}: {response.status_code} - {response.text}"
                    )
                    return False
    ...
    print(
        f"Successfully indexed {len(solr_docs)} documents to collection '{collection}'"
    )
    return True
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Index documents with both text and vector embeddings"
    )
    parser.add_argument("json_file", help="Path to the JSON file containing documents")
    parser.add_argument(
        "--collection", "-c", default="unified", help="Solr collection name"
    )
    parser.add_argument(
        "--no-commit",
        dest="commit",
