...
async def keyword_search(
    ...
    params = {"q": query, "fl": ",".join(fields), "rows": rows, "wt": "json"}
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error in keyword search: {response.status_code} - {response.text}"
                )
                return None
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                response = await client.post(
                    solr_url,
                    data={"q": params["q"]},
                    params={"fl": params["fl"], "wt": params["wt"]},
                    timeout=30.0,
                )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error in vector search: {response.status_code} - {response.text}"
                )
                return None
    except Exception as e:
...
async def hybrid_search(
    ...
    vector_results = await vector_search(
        query, collection, vector_field, fields, filter_query, k
    )
    ...
    hybrid_docs = {}
    max_keyword_score = (
        max([doc.get("score", 0) for doc in keyword_docs]) if keyword_docs else 1
    )
    max_vector_score = (
        max([doc.get("score", 0) for doc in vector_docs]) if vector_docs else 1
    )
    ...
    for doc in keyword_docs:
        ...
        normalized_score = (
            doc.get("score", 0) / max_keyword_score if max_keyword_score > 0 else 0
        )
        hybrid_docs[doc_id] = {
            **doc,
    ...
    sorted_docs = sorted(
        hybrid_docs.values(), key=lambda x: x.get("hybrid_score", 0), reverse=True
    )
...
async def main():
    ...
    parser.add_argument(
        "--results", "-k", type=int, default=5, help="Number of results to return"
    )
    ...
    if args.mode == "keyword":
        results = await keyword_search(
            args.query, args.collection, None, args.filter, args.results
        )
        if results:
        ...
        results = await vector_search(
            args.query, args.collection, "embedding", None, args.filter, args.results
        )
        if results:
