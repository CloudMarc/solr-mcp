        ...
        async with httpx.AsyncClient() as client:
            ...
            if collection_name in collections:
                ...
                delete_response = await client.get(
                    "http://localhost:8983/solr/admin/collections",
                    params={"action": "DELETE", "name": collection_name, "wt": "json"},
                    timeout=10.0,
                )

                if delete_response.status_code != 200:
                    print(
                        f"Error deleting collection: {delete_response.status_code} - {delete_response.text}"
                    )
                    return False
            ...
            if create_response.status_code != 200:
                print(
                    f"Error creating collection: {create_response.status_code} - {create_response.text}"
                )
                return False
            ...
            for field in schema_fields:
                ...
                if field_response.status_code != 200:
                    print(
                        f"Error adding field {field['name']}: {field_response.status_code} - {field_response.text}"
                    )
            ...
            if fieldtype_response.status_code != 200:
                print(
                    f"Error adding field type: {fieldtype_response.status_code} - {fieldtype_response.text}"
                )
                return False
            ...
            if vector_field_response.status_code != 200:
                print(
                    f"Error adding vector field: {vector_field_response.status_code} - {vector_field_response.text}"
                )
                return False
...
async def main():
    """Main entry point."""
    collection_name = sys.argv[1] if len(sys.argv) > 1 else "unified"
