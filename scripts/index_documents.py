...
async def index_documents(
    ...
    if collection not in collections:
        print(
            f"Warning: Collection '{collection}' not found in Solr. Available collections: {collections}"
        )
    ...
    try:
        success = await solr_client.batch_index_with_generated_embeddings(
            documents=documents, collection=collection, commit=commit
        )

        if success:
            print(
                f"Successfully indexed {len(documents)} documents in collection '{collection}'"
            )
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Index documents in Solr with vector embeddings"
    )
