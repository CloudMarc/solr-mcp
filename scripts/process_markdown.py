...
def convert_to_solr_docs(
    sections: list[tuple[str, str]], filename: str, metadata: dict
) -> list[dict]:
    """
...
def process_markdown_file(file_path: str, output_file: str = None):
    ...
    if output_file:
        ...
        print(
            f"Processed {file_path} into {len(documents)} sections and saved to {output_file}"
        )
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process markdown files for Solr indexing"
    )
