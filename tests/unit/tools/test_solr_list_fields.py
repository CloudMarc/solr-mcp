...
async def test_execute_list_fields_error(mock_server, error_message):
    """Test error handling in list_fields tool with different error messages."""
    # Configure the mock server's solr_client to raise the error
    mock_server.solr_client.list_fields.side_effect = SolrError(error_message)

    # Verify the exception is raised with the correct message
    with pytest.raises(SolrError, match=error_message):
        await execute_list_fields(mock_server, "test_collection")
